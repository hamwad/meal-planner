  ...
  <dialog ref="dialog" class="modal" :class="{ 'modal-open': isOpen }">
    <div class="modal-box max-w-2xl">
      ...
      <div v-if="activeTab === 'create'" class="py-6">
        <div v-if="!createdFamily" class="space-y-4">
          <p class="text-base-content/70">
            Create a new family to share your meal library and weekly planner
            with family members.
          </p>
      ...
      <div v-if="activeTab === 'join'" class="py-6">
        <div class="space-y-4">
          <p class="text-base-content/70">
            Enter a family code to join an existing family and access their
          ...
          <div class="alert alert-warning">
            ...
            <span class="text-sm"
              >Joining a family will replace your local meal library and
              calendar.</span
            >
          </div>
...
const { onFamilyCreated, onFamilyJoined } = useFamilySync();
...
const handleCreateFamily = async () => {
  ...
  try {
    ...
    // Handle family creation (upload existing data and prefetch)
    await onFamilyCreated(family, meals.value || [], calendarMeals.value || []);
...
const handleJoinFamily = async () => {
  ...
  try {
    // Join family
    const { family, error: joinError } = await familyService.joinFamily(
      joinCode.value,
    );
    ...
    // Handle family join (clear cache and prefetch new data)
    await onFamilyJoined(family);
