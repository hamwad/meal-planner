...
<template>
  ...
  >
        ...
        <div class="flex-1 min-w-0">
          ...
          <p class="text-xs text-surface-400 mt-0.5">
            ...
            <template v-if="meal.recipe?.prepTime || meal.recipe?.cookTime">
              ·
              {{ (meal.recipe.prepTime ?? 0) + (meal.recipe.cookTime ?? 0) }}
              min
            </template>
          </p>
        </div>
    ...
    <div class="flex flex-col gap-6 pb-6">
      ...
      <section>
        <h3
          class="text-sm font-semibold uppercase tracking-wide text-surface-400 mb-3"
        >
          Ingredients
        </h3>
      ...
      <section v-if="meal.recipe?.steps?.length">
        ...
        <ol class="flex flex-col gap-4">
          ...
          >
            <span
              class="shrink-0 w-6 h-6 rounded-full bg-primary text-primary-contrast text-xs font-bold flex items-center justify-center"
            >
              {{ index + 1 }}
            </span>
